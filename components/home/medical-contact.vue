<template>
<section class="contact-us-now bg-primary general-pattern lg:py-12 md:py-5 sm:py-4 ">
  <div class="container mx-auto">
    <div class="grid lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 mb-16">
      ...
      <div v-if="!done" class="form-content">
        ...
        <div class="form py-10">
          <div class="form-group name w-full h-12 mb-8 relative">
            <input v-model="lead.name" type="text" class="w-full h-12 :focus-within:outline-none :hover:outline-none :focus:outline-none border border-solid px-6 rounded-3xl bg-primary" :placeholder="$t(`input.name`)">

            <v-error input="name" class="mis-6 mt-2" />
          </div>

          <div class="form-group email w-full mb-8 h-12 relative">
            <input v-model="lead.mobile" type="text" class="w-full h-12 :focus-within:outline-none :hover:outline-none :focus:outline-none border border-solid px-6  rounded-3xl bg-primary" :placeholder="$t(`input.mobile`)">

            <v-error input="mobile" class="mis-6 mt-2" />
          </div>

<!--          <div class="form-group project w-full mb-10 h-12 relative">-->
<!--            <select v-model="lead.service_code" class="w-full h-12 :focus-within:outline-none :hover:outline-none :focus:outline-none border border-solid px-14 rounded-3xl bg-primary">-->
        ...
        <div class=" grid lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-9 ">
          <div class="data">
            ...
            <p class="text-links text-white font-light mt-4 text-center leading-10">
              مؤسسة تدعم المشاركة في العالم الرقمي والتواجد حيث يتواجد العملاء وتعلو المنافسة بين الشركات والنشاطات التجارية ، تقدم جميع خدمات البرمجة والتصميم والتسويق الإلكتروني .
            </p>
          </div>
